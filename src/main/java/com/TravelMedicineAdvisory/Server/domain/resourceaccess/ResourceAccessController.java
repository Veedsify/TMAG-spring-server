...
public class ResourceAccessController {
    ...
    @GetMapping
    @PreAuthorize("@perm.has(authentication, 'authorization:list', 'authorization:read')")
    public ResponseEntity<SuccessResponse> getAll(Pageable pageable) {
        Page<ResourceAccessResponse> page = service.findAll(pageable);
        ...
        PaginatedResponse<java.util.List<ResourceAccessResponse>> paginatedResponse = new PaginatedResponse<>(page.getContent(), pagination);
        return ResponseEntity.ok(new SuccessResponse("Fetched successfully", paginatedResponse));
    }

    @GetMapping("/{id}")
    @PreAuthorize("@perm.has(authentication, 'authorization:read')")
    ...
    @PostMapping
    @PreAuthorize("@perm.has(authentication, 'authorization:create')")
    public ResponseEntity<SuccessResponse> create(@RequestBody ResourceAccessRequest request) {
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(new SuccessResponse("Created successfully", service.create(request)));
    }

    @PutMapping("/{id}")
    @PreAuthorize("@perm.has(authentication, 'authorization:update')")
    public ResponseEntity<SuccessResponse> update(@PathVariable Long id, @RequestBody ResourceAccessRequest request) {
        return ResponseEntity.ok(new SuccessResponse("Updated successfully", service.update(id, request)));
    }

    @DeleteMapping("/{id}")
    @PreAuthorize("@perm.has(authentication, 'authorization:delete')")
