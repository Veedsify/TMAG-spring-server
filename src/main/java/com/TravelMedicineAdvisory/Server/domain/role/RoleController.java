...
public class RoleController {
    ...
    public ResponseEntity<SuccessResponse> getAll(Pageable pageable) {
        Page<RoleResponse> page = service.findAll(pageable);
        ...
        PaginatedResponse<java.util.List<RoleResponse>> paginatedResponse = new PaginatedResponse<>(page.getContent(), pagination);
    ...
    @PostMapping
    public ResponseEntity<SuccessResponse> create(@RequestBody RoleRequest request) {
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(new SuccessResponse("Created successfully", service.create(request)));
    }

    @PutMapping("/{id}")
    public ResponseEntity<SuccessResponse> update(@PathVariable Long id, @RequestBody RoleRequest request) {
