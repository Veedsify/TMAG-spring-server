...
@Tag(name = "Country health alerts")
@RestController
@RequestMapping("/api/v1/country-health-alerts")
public class CountryHealthAlertController {

    private final CountryHealthAlertService service;

    public CountryHealthAlertController(CountryHealthAlertService service) {
        this.service = service;
    }

    @GetMapping
    public ResponseEntity<SuccessResponse> getAll(Pageable pageable) {
        Page<CountryHealthAlertResponse> page = service.findAll(pageable);
    ...
    @PostMapping
    @PreAuthorize("@perm.has(authentication, 'country_health_alert:create')")
    public ResponseEntity<SuccessResponse> create(@RequestBody CountryHealthAlertRequest request) {
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(new SuccessResponse("Created successfully", service.create(request)));
    }

    @PutMapping("/{id}")
    @PreAuthorize("@perm.has(authentication, 'country_health_alert:update')")
    public ResponseEntity<SuccessResponse> update(@PathVariable Long id, @RequestBody CountryHealthAlertRequest request) {
        return ResponseEntity.ok(new SuccessResponse("Updated successfully", service.update(id, request)));
    }

    @DeleteMapping("/{id}")
    @PreAuthorize("@perm.has(authentication, 'country_health_alert:delete')")
