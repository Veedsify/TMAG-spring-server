...
@RestController
@RequestMapping("/api/v1/pricingplans")
public class PricingPlanController {
    ...
    public ResponseEntity<SuccessResponse> getAll(Pageable pageable) {
        Page<PricingPlanResponse> page = service.findAll(pageable);
        ...
        PaginatedResponse<PricingPlanResponse> paginatedResponse = new PaginatedResponse(page.getContent(), pagination);
    ...
    @PostMapping
    public ResponseEntity<SuccessResponse> create(@RequestBody PricingPlanRequest request) {
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(new SuccessResponse("Created successfully", service.create(request)));
    }

    @PutMapping("/{id}")
    public ResponseEntity<SuccessResponse> update(@PathVariable Long id, @RequestBody PricingPlanRequest request) {
