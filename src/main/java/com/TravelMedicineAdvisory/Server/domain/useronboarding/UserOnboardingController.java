...
@Tag(name = "User onboarding records")
@RestController
@RequestMapping("/api/v1/useronboardings")
public class UserOnboardingController {

    private final UserOnboardingService service;

    public UserOnboardingController(UserOnboardingService service) {
        this.service = service;
    }

    @GetMapping
    @PreAuthorize("@perm.has(authentication, 'user_onboarding:list')")
    public ResponseEntity<SuccessResponse> getAll(Pageable pageable) {
        Page<UserOnboardingResponse> page = service.findAll(pageable);
    ...
    @PostMapping
    @PreAuthorize("@perm.has(authentication, 'user_onboarding:create')")
    public ResponseEntity<SuccessResponse> create(@RequestBody UserOnboardingRequest request) {
    ...
    public ResponseEntity<SuccessResponse> update(@PathVariable Long id, @RequestBody UserOnboardingRequest request) {
        return ResponseEntity.ok(new SuccessResponse("Updated successfully", service.update(id, request)));
    }

    @DeleteMapping("/{id}")
    @PreAuthorize("@perm.has(authentication, 'user_onboarding:delete')")
