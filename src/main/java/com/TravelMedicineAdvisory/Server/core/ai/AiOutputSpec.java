...
/**
 * Carries both the Java type and the Gemini Schema for a structured-output call.
 * Anthropic derives its schema from {@code type} via {@code outputConfig(Class<T>)}.
 * Gemini uses the pre-built {@code geminiSchema} directly.
 *
 * @param type         the Java record class that responses are deserialised into
 * @param geminiSchema the SDK {@link Schema} object to pass in {@code responseSchema}
